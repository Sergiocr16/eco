...
17. [Pre-flight antes de cualquier PR](#preflight)
...
- **Eco es una app local-first para Mac/Win/Linux** que orquesta conversaciones con Claude. Cada conversación ("agente"/"burbuja") tiene su propio worktree git, PTY, archivos, dev server, plan, navegador. 100% local salvo la API de Anthropic.
- **Empaquetado vía Electron 33 + electron-builder 25**. Distribución: `.dmg` mac (arm64+x64), `.exe` NSIS win, `.AppImage` linux.
...
- `backend/src/dev-server.ts` — manager de sessions, spawn, kill, persistencia
- `frontend/src/components/ServerPanel.tsx` — UI (single/dual, preset workspace, logs xterm.js)
...
- `backend/src/index.ts` `/dev/start|stop|restart|status|logs|active` — endpoints
...
- `backend/src/git-ops.ts` — branches, checkout, pull, commit con AI
- `frontend/src/components/BranchPicker.tsx` — UI con dirty-changes dialog
- `frontend/src/components/CommitWithAI.tsx` — `claude -p` sugiere → preview → commit
...
# 3) Build limpio del .dmg
pkill -9 -f "Eco" 2>/dev/null
rm -rf release frontend/dist backend/dist
npm run dmg
# Output: release/Eco-0.1.0-arm64.dmg + release/mac-arm64/Eco.app
```
...
<a id="preflight"></a>
## 17. Pre-flight antes de cualquier PR
