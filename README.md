...
- **FilesPanel** — mini-VS-Code per bubble: lazy gitignore-aware tree, CodeMirror 6 editor, `Cmd+P` Quick Open, `Cmd+Shift+F` global search, save with conflict detection, image preview, and an **"↗ IDE" button** to open the current file at the exact line in VSCode / IntelliJ / WebStorm / Cursor. Eco has no built-in debugger — set breakpoints in the external IDE.
